...
1. **Inventory Sync (Frontend to Backend)**:
   - User uploads CSV/Sheets link in app; frontend POSTs to backend `/sync-inventory` (JSON: {products: [{name: "Red Dress", price: 50000, sizes: ["8"]}]}).
...
2. **Live Launch & Tagging (Frontend)**:
   - App launches camera stream (React Native Camera lib); overlays AR tags using Instagram Graph API for pinning (POST to `/media` endpoint with product IDs).
...
3. **Sell Flow & Checkout (Frontend to Backend)**:
   - Viewer taps tag → streams intent to backend `/create-checkout` (integrates TikTok Shop API for embeds if cross-posted.
